...
public class NettyClientProtoBufInitializer extends ChannelInitializer<SocketChannel> {
    ...
    protected void initChannel(SocketChannel ch) throws Exception {
        ...
        channelPipeline.addLast(new ProtobufVarint32FrameDecoder());// 用于decode前解决半包和粘包问题（利用包头中的包含数组长度来识别半包粘包）
        ...
        // ----Protobuf处理器END----
        channelPipeline.addLast( new NettyClientProtoBufHandler());//自己定义的消息处理器，接收消息会在这个类处理
       // channelPipeline.addLast(new ReadTimeoutHandler(20));
